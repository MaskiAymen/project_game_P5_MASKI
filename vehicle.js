/*
  ...
  */
  ...
  class Vehicle {
    ...
    applyLeader(target, obstacles, vehicules, distance, isLeader) {
      if (isLeader) {
        let seekForce = this.arrive(target, distance);
        let avoidForceObstacles = this.avoid(obstacles);
        // let avoidForceVehicules = this.avoidVehicules(vehicules);
        ...
        let leaderBehind = p5.Vector.sub(vehicules[0].pos, this.vel.copy().setMag(120));
        let distanceToLeader = this.pos.dist(vehicules[0].pos);
        let avoidanceCircleCenter = vehicules[0].pos.copy().add(vehicules[0].vel.copy().setMag(120));
        let distanceToAvoidanceCircle = this.pos.dist(avoidanceCircleCenter);
        
    
        if (distanceToAvoidanceCircle < 70) {
          let avoidForce = p5.Vector.sub(this.pos, avoidanceCircleCenter);
    ...
    */
    ...
    wander(obstacles) {
      ...
      // Avoid obstacles during wander
      let avoidForceObstacles = this.avoid(obstacles);
      avoidForceObstacles.mult(1.5); // Adjust the multiplier as needed
      this.applyForce(avoidForceObstacles);
    
      let desiredSpeed = p5.Vector.sub(wanderPoint, this.pos);
  ...
  class Target extends Vehicle {
    ...
    show() {
      ...
      fill(255);
      noStroke();
      ellipse(this.pos.x, this.pos.y, this.r * 2 + 20); // Adjust the padding as needed
